...
impl<K, V> Node<K, V> {
    fn len(ptr: &NodePtr<K, V>) -> usize {
        if let &Some(ref b) = ptr {
            1 +Node::len(&b.left) + Node::len(&b.right)
        } else {0}
    }
}

impl<K: Ord, V> Node<K, V> {
    pub fn find_rec<'a>(ptr: &'a NodePtr<K, V>, key: &K) -> Option<&'a V> {
    ...
    fn find_mut_rec<'a>(ptr: &'a mut NodePtr<K, V>, key: &K)
        -> Option<&'a mut V>
    {
    ...
    fn find_iter<'a>(mut ptr: &'a NodePtr<K, V>, key: &K) -> Option<&'a V> {
